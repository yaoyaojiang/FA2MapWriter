...
class CreateTriggers extends JFrame {
    ...
    private JTextField enums = new JTextField("在此填写枚举类型内容");
    ...
    public CreateTriggers() throws InterruptedException, IOException{
        ...
        owner.setName("owner");
        create.setBounds(70, 360, 120, 30);
    ...
    private void writeTriggers(mapList list) throws Exception {
        ...
        if(createnum.getText().equals("%enum%")) num=e.length;
        else num=Integer.parseInt(createnum.getText());
        ...
        for(i=1;i<=num;i++)
        {
            ...
            if(i>e.length)
            {
                nametext=nametext.replaceAll("%enum%",e[e.length-1]);
            }
            else
            {
                nametext=nametext.replaceAll("%enum%",e[i-1]);
            }
            String nametext2="";
