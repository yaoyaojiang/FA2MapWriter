...
public class CommonMethod {
    ...
    public mapList readMapList(String fileName) throws IOException
    {
        ...
        while (scanner.hasNextLine()) {
            ...
            {
                list.setPlayerHouse(line.split("=")[1]);
            }
            if (start > 0) {
                if (line.indexOf("=") > -1) {
                    if (start == 1) {
                    ...
                    {
                        code = line.replace(" ", "").split("=")[1];
                        list.addVariableNames(code);
                    }
                    else if (start==14)
                    {
                        String house=line.split("=")[1].split(",")[0];
                        String struc=line.replace(" ", "").split("=")[1].split(",")[1];
                        if((struc.equals("GACNST")||struc.equals("NACNST")||struc.equals("YACNST"))&&(!house.equals(list.getPlayerHouse())))
                        {
                            if(house.contains(" ")) house=house.replaceAll("\\s.*", "");
                            list.setUsingSPYPHouse(house);
                        }
                    }
            ...
            else if (line.contains("[VariableNames]")){
                start=13;
            }
    ...
    public iniList readini(File file) throws FileNotFoundException {
        Scanner scanner=new Scanner(file);
        iniList list=new iniList();
        list.initialize();
        int lineno=1;
        List<String> tagline=new ArrayList<String>();
        int start=0;
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            if(line.startsWith("[")&&line.endsWith("]"))
            {
                tagline.add(lineno+","+line.split("\\[")[1]+line.split("]")[0]);
                if(line.startsWith("[SuperWeaponTypes]"))
                {
                    start=1;
                }
                else if (line.startsWith("[Warheads]"))
                {
                    start=2;
                }
                else if (line.startsWith("[BuildingTypes]"))
                {
                    start=3;
                }
                else if (line.startsWith("[AircraftTypes]"))
                {
                    start=4;
                }
                else if (line.startsWith("[VehicleTypes]"))
                {
                    start=5;
                }
                else if (line.startsWith("[InfantryTypes]"))
                {
                    start=6;
                }
                else if (line.startsWith("[VariableNames]"))
                {
                    start=7;
                }
                else start=0;
            ...
            else if (start>0)
            {
                if (start==1)
                {
                    list.addSuperWeapons(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==2)
                {
                    list.addWarheads(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==3)
                {
                    list.addBuildings(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==4)
                {
                    list.addAircrafts(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==5)
                {
                    list.addVehicles(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==6)
                {
                    list.addInfantry(line.split("=")[0]+","+line.split("=")[1]);
                }
                else if (start==7)
                {
                    list.addVariables(line.split("=")[0]+","+line.split("=")[1]);
                }
            }
            lineno++;
        }
        list.setTagLine(tagline);
        return list;
    }
